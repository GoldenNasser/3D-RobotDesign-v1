...
Body Shape & Structure: A simple, lightweight rectangular chassis. This design provides a flat, stable platform to securely mount the system's core components (microcontroller, sensors, and battery) while keeping the Center of Gravity (CoG) predictable.
...
Joints & Degrees of Freedom (DOF): The system utilizes 2 joints per leg (Shoulder/Hip and Knee), totaling 8 joints for the entire robot (8 DOF). Keeping the DOF relatively low acts as a system constraint to optimize battery consumption and simplify the programming logic.
...
## Risk Management (Expected Mechanical Problems & Solutions)

* **Problem 1 (Friction):** High friction between the upper and lower leg segments at the knee joint.
* **Mitigation 1:** Introduce small washers or mechanical spacers between the segments to allow smooth rotation and reduce motor strain.
* **Problem 2 (Weight Imbalance):** The robot tipping backward when accelerating.
* **Mitigation 2:** Rearrange the internal hardware components to shift the Center of Gravity slightly forward, compensating for the backward momentum.
